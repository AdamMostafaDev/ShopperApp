...
export async function POST(
  ...
  try {
    ...
    const paymentConfirmationData = {
      ...
      // USD equivalents
      productCostUsd: amounts.productCostBdt / (order.exchangeRate || 121.5),
      serviceChargeUsd: amounts.serviceChargeBdt / (order.exchangeRate || 121.5),
      shippingCostUsd: amounts.shippingCostBdt / (order.exchangeRate || 121.5),
      taxUsd: amounts.taxBdt / (order.exchangeRate || 121.5),
      totalAmountUsd: amounts.totalAmountBdt / (order.exchangeRate || 121.5),
    ...
    const result = await sendPaymentConfirmationEmail(paymentConfirmationData);

    if (result.success) {
      console.log('✅ Payment confirmation email sent successfully for order:', orderId);
      return NextResponse.json({
        success: true,

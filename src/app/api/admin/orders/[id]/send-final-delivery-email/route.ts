...
import { sendFinalDeliveryConfirmationEmail } from '@/lib/klaviyo';
...
export async function POST(
  ...
  try {
    ...
    // Check if delivered status is DELIVERY_COMPLETE
    if (order.deliveredStatus !== 'DELIVERY_COMPLETE') {
      console.log(`⚠️ Order ${orderId} is not in DELIVERY_COMPLETE status`);
      return NextResponse.json({
        success: false,
        error: `Final delivery confirmation email can only be sent when delivered status is DELIVERY_COMPLETE. Current status: ${order.deliveredStatus}`
      }, { status: 400 });
    }
    ...
    console.log(`✅ Sending final delivery confirmation email to ${customerEmail} for order ${orderId}`);
    ...
    const result = await sendFinalDeliveryConfirmationEmail({
      customerEmail,
      customerName,
    ...
    return NextResponse.json(
      {
        success: false,
        error: error instanceof Error ? error.message : 'Failed to send final delivery confirmation email'
      },
      { status: 500 }
    );
  }
